...
test.describe('TC-TS-COMMENT-06 — Delete EMR comment', () => {
  ...
  test('Delete EMR comment with confirmation', async ({ page }) => {
    await test.step('Setup: Delete any leftover test EMR comment from a prior run', async () => {
      ...
      if (await showAllBtn.isVisible()) {
        ...
        const leftover = historyDialog.locator('table tbody tr', { hasText: TEST_EMR_COMMENT });
