...
namespace Crt.Domain.Services
{
    ...
    public class KeyCloakService : IKeycloakService
    {
        ...
        public KeyCloakService(IConfiguration config, IHttpClientFactory httpClientFactory, CrtCurrentUser currentUser, ILogger<KeyCloakService> logger, IUserRepository userRepo, IUnitOfWork unitOfWork)
        {
            _config = config;
        ...
        private string GetUniqueToken(int length, string chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890")
        {
            using (RNGCryptoServiceProvider crypto = new RNGCryptoServiceProvider())
            {
                ...
                return $"{Constants.AppName}.{new string(result)}";
            }
        }
    }
}
